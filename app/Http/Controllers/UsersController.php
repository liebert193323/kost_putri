...
class UsersController extends Controller
{
    public function index()
    {
        ...
        $users = $user->account->users() // Ambil data pengguna berdasarkan account
            ->orderByName()
            ->filter(Request::only('search', 'role', 'trashed'));
    
        // Pastikan hanya data yang sesuai role yang diambil
        if ($user->owner) {
            $users = $users->where('owner', true);  // Filter untuk owner
        } else {
            $users = $users->where('owner', false); // Filter untuk penghuni
        }
    
        return Inertia::render('Users/Index', [
            'filters' => Request::all('search', 'role', 'trashed'),
            'users' => $users->paginate()->transform(function ($user) {
                return [
                    'id' => $user->id,
                    'name' => $user->name,
                    'email' => $user->email,
